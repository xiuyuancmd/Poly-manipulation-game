...
菜单顶部一键切换两套表现层皮肤（`localStorage` 记忆，默认工程主题）：

- **软体试验台**（lab，默认）：工程质检语境——硅胶试件、管路、切割/粘合，工业音效。
- **活体标本**（bio）：同一套物理与判定的生物镜像——组织红配色、血管/肌腱/血窦文案、
  手术刀/缝合工具、湿噗与心跳音效；充压腔体带脉搏起伏，**放血后就不跳了**；
  泄压粒子变暗红液滴，脆断碎屑变骨白钙化渣。

主题只动文案、配色、粒子与音效（见 `src/game/themes.js`）；
相似度、判定与物理规则两主题完全一致。
